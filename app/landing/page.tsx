...
const LandingPage: React.FC = () => {
  ...
  const addToMakersFeed = async (newListing: MakerItem) => {
    try {
      ...
      if (response.ok) {
        ...
        setMakersFeed((prevFeed) => [addedListing.listing, ...prevFeed]); // Add new listing at the top
  ...
  const addToCollectorsFeed = async (newListing: CollectorItem) => {
    try {
      ...
      if (response.ok) {
        const addedListing = await response.json();
        setCollectorsFeed((prevFeed) => [addedListing, ...prevFeed]); // Add new listing at the top
        setConfirmationMessage("Listing added successfully!");
        setTimeout(() => setConfirmationMessage(null), 3000); // Clear message after 3 seconds
      } else if (response.status === 409) {
        console.error("Duplicate listing detected");
  ...
  const sendClaimEmailNotification = async (
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(
          "An email has been sent to notify ${firstName} that their item has been claimed."
        );
      } else {
    ...
    try {
      ...
      if (response.ok) {
        setCollectorsFeed((prevFeed) =>
          prevFeed.filter((item) => item.id !== id)
        );
        console.log("Listing deleted successfully");
      } else {
        console.error("Failed to delete listing");
    ...
    <div className="min-h-screen bg-white text-black flex flex-col">
      ...
      <div className="text-center p-6 bg-[rgba(240,191,34,0.5)] border-b border-gray-300">
        <h1 className="text-4xl font-semibold">
          Join the Junky Jamboree with Our Material Collection!
        </h1>
        <p className="text-lg mt-2">
          Our Trash Smashing artisans are accepting specific reusable materials
          at the Trashy Holiday Market (Nov 29-30). Accepted Materials will be
          transformed into future one-of-a-kind goods. Attendees are invited to
          gather and give requested materials to the Makers at the Trashy
          Market. This material matching site is a new experiment for us, so if
          you run into any technical issues or have feedback, please contact
          Katy@nowhere-collective.com.
        </p>
      </div>
      ...
      <div className="flex flex-grow flex-row">
        ...
        <div className="w-1/2 p-8 bg-white border-r border-gray-300">
          <h2 className="text-2xl font-bold mb-4">
            Trashy Makers: Material Requests{" "}
          </h2>
          <p className="text-base text-gray-800 mb-6">
            If you are a Trashy Maker, fill out the form below to post your
            material requests. Check the Offers feed by November 28 to claim
            your materials and notify the Giver that you will accept their
            materials!
          </p>
          <button
            onClick={(e) => {
          ...
          <div className="mt-8">
            {selectedForm === "form1" && (
              <FormOne addToFeed={addToMakersFeed} closeModal={closeModal} />
            )}
        ...
        <div className="w-1/2 p-8 bg-white">
          <h2 className="text-2xl font-bold mb-4">
            Gather & Give: Material Offers
          </h2>
          <p className="text-base text-gray-800 mb-6">
            Attention Attendees of Trashy Holiday! Scroll through the Trashy
            Makers' material requests (on the left). If you potentially have
            material that match their request, complete the form below. If you
            have materials that aren't a match but might be intriguing, feel
            free to post too. By November 28th, the Trashy Makers will claim
            their preferred materials. If your material is claimed, you will
            receive an email inviting you to bring it to the Maker at the Trashy
            Market (Mrs. Murphy's Irish Bistro 3905 N Lincoln Ave).
          </p>
          <button
            onClick={(e) => {
              e.preventDefault();
                ...
                {collectorsFeed.map((item) => (
                  ...
                  >
                    ...
                    <p>
                      ...
                      {item.marketStatus || "Still Available"}
                    </p>
