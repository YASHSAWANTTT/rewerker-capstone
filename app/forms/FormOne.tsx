...
const FormOne = ({ addToFeed }: { addToFeed: (item: any) => void }) => {
  const [imageBase64, setImageBase64] = useState<string | null>(null); // Store base64 image
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      // Ensure the image is of supported type
      ...
      reader.onloadend = () => {
        ...
        setImageBase64(base64String); // Set the base64 string
        setImageUrl(URL.createObjectURL(file)); // Set local preview
      };
  ...
  const handleGPTGeneration = async () => {
    ...
    try {
      const response = await fetch('/api/generate-description', {
        ...
        body: JSON.stringify({ imageBase64 }), // Send base64 string to the backend
      });
